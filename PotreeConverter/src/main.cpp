...
PotreeArguments parseArguments(int argc, char **argv) {
    ...
    args.addArgument("input-format,f",
                     "Input format. xyz: cartesian coordinates as floats, rgb: colors as numbers, i: intensity as number");
    ...
    args.addArgument("description", "Description");
    args.addArgument("autor-page", "Page Autor");
    args.addArgument("description-page", "Description to be shown in the page.");
...
//	args.addArgument("show-skybox", "");
    ...
    args.addArgument("material",
                     "RGB, ELEVATION, INTENSITY, INTENSITY_GRADIENT, CLASSIFICATION, RETURN_NUMBER, SOURCE, LEVEL_OF_DETAIL");
    args.addArgument("store-size",
    ...
    if (args.has("aabb")) {
        ...
        for (size_t p = 0, q = 0; p != strAABB.npos; p = q)
            aabbValues.push_back(
                    atof(strAABB.substr(p + (p != 0), (q = strAABB.find(sep, p + 1)) - p - (p != 0)).c_str()));
    ...
    if (args.has("description-page")) {
        a.descriptionPage = args.get("description-page").as<string>();
    }
...
//	a.backgroundType = args.has("background-type");
    ...
    vector<string> validMaterialNames = {"RGB", "ELEVATION", "INTENSITY", "INTENSITY_GRADIENT", "CLASSIFICATION",
    ...
    if (args.has("csvdir")) {
        a.csvdir = args.get("csvdir").as<string>();
    } else {
        a.csvdir = a.source[0];
    }
...
void printArguments(PotreeArguments &a) {
    try {
        cout << "========================" << endl;
        cout << "NOUMENA" << endl;
        cout << "pointcloud-webgl-converter" << endl;
        cout << "== params ==" << endl;
        int i = 0;
